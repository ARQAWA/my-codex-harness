...
The root agent orchestrating this task owns fixes and completion. It invokes the `spotty` reviewer directly through the `Agent` tool; do not add another manager or role layer for the check.
...
For every attempt, spawn a fresh `spotty` subagent through the `Agent` tool with the frozen packet as the prompt, at medium effort. Never reuse or resume a closed or interrupted reviewer instance: each pass uses a new instance. If the `Agent` tool or the `spotty` profile is unavailable, the selected check is unavailable and cannot pass; root-agent self-review is not a substitute. It checks only substantial requirement→object/evidence and concrete change→authority mismatches under the reviewer's materiality gate.

If a reviewer hang is confirmed, stop that pass and spawn a fresh `spotty` instance on the unchanged object. Never resume a closed or interrupted reviewer.
...
findings that satisfy the reviewer's materiality gate. If no findings
...
packet, and spawn a new fresh Spotty. Repeat autonomously until one pass
