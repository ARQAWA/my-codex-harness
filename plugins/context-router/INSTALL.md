...
Готовность означает: новый чат использует заданные catalog/instructions, если они подключены; доступны `new_context` и `get_context_remaining`; checkpoint сохраняется и восстанавливается через bootstrap для правильного thread. Устанавливающий агент проходит один штатный переход с сохранённым checkpoint и подтверждает восстановление; различает fresh window и server summary по исходным записям. Для пути без injected bootstrap используется один CLI bootstrap. Если версия Codex не поддерживает необходимую возможность, сообщи ограничение: установленный пакет и записанный config сами по себе не доказывают работу переключения.
