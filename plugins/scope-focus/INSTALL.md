...
а hooks передают предусмотренные инструкции. Если подключены custom catalog
и instructions, новый чат использует выбранные файлы. Locator Task Notebook
и новый HTML ведут внутрь временной папки ОС; наличие notebook не активирует
его без выбора пользователя. Очистка ОС может удалить временные материалы.
