...
Use only the installed `codebase-memory-mcp cli` through the CBM wrappers. The search wrapper finds the project identity, creates a missing index, and waits for it. With a usable graph it returns queries while one packaged Rust daemon maintains that root. The index wrapper explicitly waits for refresh. Do not maintain agent-local identity or dirty flags, manually enumerate projects, or call `index_status`. Use `check_index_coverage` only for a concrete coverage question.

CBM uses its own central store, no project-local persistence. A packaged Rust watcher uses CLI `index_repository --mode full`; CBM owns the incremental/full decision. It watches the canonical project root recursively, with or without Git, and coalesces filesystem events for 5 seconds. CBM's native indexer applies `.gitignore`, `.cbmignore`, and its standard exclusions, including for non-Git roots. Existing graphs may briefly lag; an update failure is reported as `CBM_INDEX_STALE`, never as a fresh result. A missing graph or unavailable daemon exits `75`. No MCP server is started.

Tgrep uses its official `serve` and central `<active-codex-home>/tgrep/index/<hash>/`. The first call waits for complete indexing under tgrep's standard ignore rules and a live watcher, with no one-second indexing cutoff. Indexing follows those rules, including `.gitignore` outside Git repositories and `.ignore`; the wrapper excludes `.git` and does not enable `--no-ignore`. Explicit `--hidden` searches hidden files while preserving ignore rules and the `.git` exclusion, subject to native type/size limits. Exact file scopes, explicit `--no-ignore`, and unsupported indexed modes use the compatibility route. Native tgrep may use its on-disk index if the server disappears; no absolute index-only guarantee is claimed for exceptional backend behavior.
