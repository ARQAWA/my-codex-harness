...
При заказанной установке или обновлении плагина обновлять весь его runtime:
пакет в глобальном пользовательском `.codex`, packaged agent profiles и
отдельные runtime-файлы, если они изменились. Сохранять настройки и данные.
Не считать исходники или запись marketplace доказательством обновлённого runtime.

Перед любой заказанной установкой или обновлением сначала полностью читать
соответствующий `INSTALL.md` и setup skill. Установка считается завершённой
только после всех описанных частей runtime; одной команды `plugin add` мало.

Установку с нуля и обычное обновление описывать отдельно. Полные установочные
runtime-проверки относятся к установке с нуля. При накате правок не повторять
их без прямого запроса; читать изменённый результат и нормальный вывод установки.
Новые файлы не означают замену уже загруженного контекста открытой задачи.

В документации и описаниях использовать стандартный пользовательский каталог
`.codex` и реальные пути хоста. Не упоминать переменную окружения,
переопределяющую корень Codex, ни в кодовом написании, ни словами.
Для Windows использовать только Git Bash.
