...
указание на него в описании plugin.json. Имена файлов и их расположение должны
совпадать.

INSTALL.md и справочные тексты должны быть короткими и содержательными.
INSTALL.md описывает состав пакета, отдельно подключаемые части, готовность
первой установки и обычное обновление. Общие фразы не заменяют эти сведения.
Команды, переменные окружения и пошаговые технические рецепты не нужны.
Агент сам определяет способ работы для текущей системы.
